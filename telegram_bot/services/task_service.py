...
class TaskService:
    ...
    def create_task_from_scheduler(self, schedule: ScheduledTask) -> Task or None:
        """
        ...
        self.upsert_task(new_task)

        # если задача имеет тип Однократно - значит это задача с отложенным запуском. Удаляем schedule из кеша
        if schedule.task_type == TaskType.ONCE:
            self.scheduler_task_cache.pop(task_id, None)
        else:

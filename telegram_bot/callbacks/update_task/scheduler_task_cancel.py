...
@router.callback_query(F.data.startswith("scheduler_task:cancel:"))
async def scheduler_task_cancel_request(callback: CallbackQuery):
    """Колбек с прослойкой из нопок подтверждения. Старый колбек закомменчен"""

    scheduler_task_id = callback.data.split(":")[2]

    await callback.message.edit_text(
        "Вы уверены, что хотите удалить задачу по расписанию?",
        reply_markup=confirm_keyboard("scheduler_task:cancel", scheduler_task_id)
    )
